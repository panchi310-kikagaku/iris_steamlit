...
if st.sidebar.button("Predict"):
    ...
    # 予測結果の出力
    iris_name = str(targets[int(prediction)])
    st.write('## Result')
    st.write('このアイリスはきっと', iris_name,'です!')

    # 画像の表示
    img_path = './img/' + iris_name + '.png'  # 拡張子を含めたパスに変更
    img = Image.open(img_path)
    st.image(img, caption=iris_name , use_column_width=True)
